...
function renderRecipe(recipe) {
  ...
  // 🔥 [수정] 무조건 실제 리뷰 배열을 가져와서 개수 세기
  // (JSON에 잘못된 review_count가 있어도 무시함)
  ...
  if (ratingEl) {
    ...
    // 별점 + 리뷰 텍스트 출력 (정확한 reviewCount 사용)
...
function setupReviewSystem(recipe) {
  ...
  if (reviewBtn) {
    reviewBtn.addEventListener('click', () => {
      ...
      if (emptyMsg) {
        emptyMsg.remove(); // 안내 문구가 존재하면 태그 자체를 삭제!
      }
      ...
      // (선택) 리뷰 등록 후 상단 별점 텍스트도 +1 시켜주면 완벽하지만,
      // 현재 구조상 여기까지만 해도 충분합니다.
